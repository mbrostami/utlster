...
func clientHandshake(uConn *tls.UConn, clientHello []byte) error {
	...
	for _, extension := range generatedSpec.Extensions {
		log.Debug().Msgf("client hello extension %+v", extension)
		if sni, ok := extension.(*tls.SNIExtension); ok {
			uConn.SetSNI(sni.ServerName)
		}
	}
...
func serveTLS(l net.Listener, tlsConfig *tls.Config) error {
	for {
		...
		if err != nil {
			return fmt.Errorf("tcp/tls accept read failed: %v", err)
		}
		log.Debug().Msg("server handshake done")
	}
}

func handshake(rAddr *net.TCPAddr, clientHello []byte) error {
	...
	defer uConn.Close()
	if err := clientHandshake(uConn, clientHello); err != nil {
